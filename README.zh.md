...
- **工作坊实时工具**：实时聊天/私信、求助队列与仅限证书持有者的抽奖轮盘。
- **双向实时屏幕共享**：组织者可以将自己的屏幕广播给所有参与者，同时在实时网格视图中监控所有参与者的屏幕。支持参与者的可调大小 PiP 和组织者的全屏放大 (Enlarge) 功能。
...
## 🧭 组织者工具包（新增）
- **Live 标签页**：参与者进度实时追踪、聊天/私信与求助队列。
- **屏幕共享监控**：通过可定制的网格查看所有参与者的屏幕。放大特定流以进行详细的技术指导。
- **测验 & 反馈**：设置结业条件并汇总结果。
