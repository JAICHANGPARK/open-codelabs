<script lang="ts">
    ...
    >
        <div class="max-w-screen-2xl mx-auto flex flex-col gap-3">
            <div class="flex justify-between items-center gap-2 sm:gap-3">
                ...
                <div class="flex items-center gap-1 sm:gap-2 lg:gap-4 shrink-0">
                    ...
                    <button
                        ...
                        class="relative inline-flex h-6 w-10 sm:h-7 sm:w-12 shrink-0 cursor-pointer items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[#4285F4] focus:ring-offset-2 {codelab?.is_public ? 'bg-[#34A853]' : 'bg-gray-200 dark:bg-dark-border'}"
                        role="switch"
                        aria-checked={codelab?.is_public != 0}
                        title={codelab?.is_public ? $t("common.public") : $t("common.private")}
                    >
                        <span
                            class="pointer-events-none flex h-4 w-4 sm:h-5 sm:w-5 items-center justify-center rounded-full bg-white shadow-sm ring-0 transition-transform duration-200 {codelab?.is_public ? 'translate-x-5 sm:translate-x-6' : 'translate-x-1'}"
                        >
                            {#if codelab?.is_public}
                                <Eye size={10} class="text-[#34A853] sm:w-3 sm:h-3" />
                            {:else}
                    ...
                    >
                        {#if isSaving}
                            <Loader2 size={16} class="animate-spin sm:w-4.5 sm:h-4.5" />
                        {:else if saveSuccess}
            ...
            <div class="flex items-center gap-1 overflow-x-auto no-scrollbar pb-1 -mx-3 px-3 sm:mx-0 sm:px-0">
                <div
