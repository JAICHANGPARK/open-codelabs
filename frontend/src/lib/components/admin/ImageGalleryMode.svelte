<script lang="ts">
    ...
    let imageSubmissions = $derived(
        submissions.filter(sub => isImage(sub.file_name, sub.submission_type))
    );

    let filteredImages = $derived(
        imageSubmissions.filter(s =>
...
<div class="space-y-6" in:fade>
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-accent/60 dark:bg-white/5 p-6 rounded-2xl border border-border dark:border-dark-border">
        <div>
            ...
            <p class="text-sm text-muted-foreground dark:text-dark-text-muted mt-1">
                {imageSubmissions.length} {$t("gallery.image_count")}
            </p>
        </div>

        <div class="flex items-center gap-3">
            <!-- View Mode Toggle -->
            <div class="flex items-center bg-white dark:bg-dark-surface rounded-full border border-border dark:border-dark-border p-1">
                <button
                    onclick={() => viewMode = "grid"}
                    class="p-2 rounded-full transition-all {viewMode === 'grid' ? 'bg-primary text-white' : 'text-muted-foreground hover:text-foreground'}"
                ...
                <button
                    ...
                    class="p-2 rounded-full transition-all {viewMode === 'masonry' ? 'bg-primary text-white' : 'text-muted-foreground hover:text-foreground'}"
    ...
    {#if filteredImages.length === 0}
        <div class="flex flex-col items-center justify-center py-20 text-center bg-white dark:bg-dark-surface rounded-3xl border-2 border-dashed border-border dark:border-dark-border">
            ...
            <h3 class="text-lg font-bold text-foreground dark:text-dark-text">{$t("gallery.no_images")}</h3>
            <p class="text-sm text-muted-foreground dark:text-dark-text-muted mt-2">
                {searchTerm ? $t("gallery.no_search_results") : $t("gallery.waiting_images")}
            </p>
        </div>
    {:else}
        {#if viewMode === "grid"}
                ...
                {#each filteredImages as img, i}
                    {#if isHeic(img.file_name)}
                        ...
                        >
                            ...
                            <p class="text-xs font-bold text-foreground dark:text-dark-text text-center truncate w-full">{img.attendee_name}</p>
                            <p class="text-[10px] text-muted-foreground dark:text-dark-text-muted text-center truncate w-full mt-1">{img.file_name}</p>
                            <p class="text-[9px] text-primary font-medium mt-2 bg-primary/10 px-2 py-1 rounded-full">HEIC - {$t("common.download")}</p>
                        </a>
                    {:else}
                        <button
                        ...
                        >
                            ...
                            <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                                <div class="absolute bottom-0 left-0 right-0 p-3 text-white">
                                    <p class="text-xs font-bold truncate">{img.attendee_name}</p>
                                    <p class="text-[10px] opacity-80 truncate">{img.file_name}</p>
                                </div>
                            </div>
                        </button>
                    {/if}
...
{#if selectedImage}
    ...
    >
        ...
        <div class="flex flex-col items-center max-w-5xl max-h-[90vh] w-full" onclick={(e) => e.stopPropagation()}>
            {#if selectedImage && isHeic(selectedImage.file_name)}
                <!-- HEIC file - show download prompt -->
                <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-12 text-white text-center">
                    ...
                    <p class="text-sm opacity-70 mb-6">This image format is not supported by browsers.<br>Please download to view.</p>
                    <a
            ...
            <div class="mt-6 bg-white/10 backdrop-blur-sm rounded-2xl p-4 text-white max-w-2xl w-full">
                <div class="flex items-center justify-between gap-4">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 text-sm mb-1">
                            <User size={14} />
                            <span class="font-bold">{selectedImage.attendee_name}</span>
                        </div>
                        <p class="text-xs opacity-70 truncate">{selectedImage.file_name}</p>
