...
`oc mcp serve`는 현재 `oc` profile과 session을 재사용해서 stdio MCP 서버를 띄웁니다. 그래서 Claude Desktop, Codex, Cursor 같은 MCP host에서 codelab, guide, steps를 바로 읽을 수 있고, 퍼실리테이터/작성자용 prompt 템플릿도 바로 재사용할 수 있으며, 관리자 세션이 있으면 일부 관리 작업도 수행할 수 있습니다. 자세한 명령 설명은 [CLI 레퍼런스](docs/user-guide/cli.md), host 설정 예시는 [MCP 서버 가이드](docs/user-guide/mcp.md)를 참고하면 됩니다.
...
Docker Compose에서 PostgreSQL도 함께 띄우려면 기본 `.env`는 그대로 두고 override 파일과 함께 실행하면 됩니다. override 파일이 backend를 내장 `postgres` 서비스로 연결하며, 세부 값은 `.env.sample`의 `POSTGRES_*` 항목으로 조정할 수 있습니다.
...
# 또는 PostgreSQL 사용: DATABASE_URL=postgresql://postgres:postgres@localhost:5432/open_codelabs
...
**Backend**
- `DATABASE_URL`: SQLx 연결 문자열 (sqlite/postgres). 예: `sqlite:/app/data/sqlite.db?mode=rwc`, `postgresql://postgres:postgres@postgres:5432/open_codelabs`.
...
- `POSTGRES_DB` / `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_PORT` / `POSTGRES_DATA_PATH`: `docker-compose.postgres.yml`에서 사용하는 값입니다.
...
- **백업 및 복구**: 관리자 패널에서 시스템 전체 상태를 손쉽게 내보내고 가져올 수 있습니다.
