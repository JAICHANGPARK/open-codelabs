...
use backend::domain::models::{Codelab, CreateCodelab, CreateStep, Step, UpdateStepsPayload};
use backend::infrastructure::db_models::AuditLog;
use serde::Serialize;
...
fn help_error(topic: &str) -> anyhow::Error {
    anyhow!("Usage help requested for `{topic}`. Run `oclabs help` for command overview.")
}

fn print_help() {
    println!(
        r#"Open Codelabs administrative CLI

Usage:
  oclabs [global options] <command>
