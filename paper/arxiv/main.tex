...
Empirically, this integration demonstrates sub-200ms real-time event distribution up to 100 concurrent users per node, and our staged Pro-mode AI reduces structural generation omissions by 27\% relative to a single-pass baseline.
...
The logical implication $q_c \Rightarrow Q_{i,c}$ explicitly enforces that if a requirement is enabled ($q_c=1$), fulfillment ($Q_{i,c}=1$) is strictly demanded; otherwise, the condition is transparent. This is algebraically equivalent to evaluating $1 - q_c(1 - Q_{i,c})$.
...
\begin{figure}[p]
\centering
\IfFileExists{figures/fig4-db-learning.pdf}{
\includegraphics[width=0.98\linewidth]{figures/fig4-db-learning.pdf}
}{
\IfFileExists{figures/fig4-db-learning.png}{
\includegraphics[width=0.98\linewidth]{figures/fig4-db-learning.png}
}{
\resizebox{0.98\linewidth}{!}{
\begin{tikzpicture}[
  x=1.0cm, y=1.0cm,
...
\caption{Core database structure (a) learning-flow domain, derived from migration DDL (first-order codelab-scoped foreign keys).}
\label{fig:db_erd}
\end{figure}

\begin{figure}[p]
\centering
\IfFileExists{figures/fig4-db-realtime.pdf}{
\includegraphics[width=0.98\linewidth]{figures/fig4-db-realtime.pdf}
}{
\IfFileExists{figures/fig4-db-realtime.png}{
\includegraphics[width=0.98\linewidth]{figures/fig4-db-realtime.png}
}{
\vspace{2em}\textit{Realtime ERD image not found.}\vspace{2em}
}
}
\caption{Core database structure (b) realtime-operation domain.}
\label{fig:db_erd_realtime}
\end{figure}

\begin{figure}[p]
\centering
\IfFileExists{figures/fig4-db-assessment-ai.pdf}{
\includegraphics[width=0.98\linewidth]{figures/fig4-db-assessment-ai.pdf}
}{
\IfFileExists{figures/fig4-db-assessment-ai.png}{
\includegraphics[width=0.98\linewidth]{figures/fig4-db-assessment-ai.png}
}{
\vspace{2em}\textit{Assessment/AI ERD image not found.}\vspace{2em}
}
}
\caption{Core database structure (c) assessment, AI, and workspace domain.}
\label{fig:db_erd_assessment_ai}
\end{figure}
...
Frontend unit tests (\texttt{bun test --coverage}) & pass (exit 0) & Local refresh run: 52 tests, 8.73 s. Note: extensive UI/E2E coverage is deferred to future work. \\
...
\begin{itemize}[leftmargin=1.2em]
  ...
  \item automated frontend test coverage focuses on utilities rather than extensive UI component or end-to-end (E2E) state transitions across the 15 facilitator modes;
